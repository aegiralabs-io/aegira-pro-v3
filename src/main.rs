...
fn send_gmail_alert(subject: &str, body: &str) -> Result<(), String> {
    ...
    .ok_or_else(|| {
        "Composio user ID is not configured. Set COMPOSIO_USER_ID or configure alerts.composio_user_id."
            .to_string()
    })?;
    ...
    .ok_or_else(|| {
        "Alert recipient is not configured. Set AEGIRA_ALERT_EMAIL or configure alerts.recipient_email."
            .to_string()
    })?;
